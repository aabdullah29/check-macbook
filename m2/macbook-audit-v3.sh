...
# Apple sales/configuration number.
# macOS may call this Model Number, Part Number, or Order Number.

MODEL_NUMBER="$(echo "$HW" | awk -F': ' '
  /^[[:space:]]*Model Number:/ {
    print $2
    exit
  }
...
MODEL_NUMBER_LABEL="$(echo "$HW" | awk -F': ' '
  /^[[:space:]]*Model Number:/ {
    print "Model Number"
    exit
  }
...
echo "Model / Part No.   : ${MODEL_NUMBER:-Not reported by macOS}"
echo "Identifier Label   : ${MODEL_NUMBER_LABEL:-Not reported}"
...
echo
echo "MANUAL ACTION:"
echo "If region/original sales configuration matters:"
echo
echo "- Check the original Apple box label for the Part/Order Number."
echo "- Check the original invoice if available."
echo "- Do not treat Model Identifier (e.g. MacBookPro18,3) as the Product/Part Number."

echo
echo "Apple Check Coverage:"
echo "https://checkcoverage.apple.com/coverage?locale=en_US"

echo
echo "ACTION:"
echo "1. Open the Apple Check Coverage website."
echo "2. Enter the Mac serial number shown above."
echo "3. Verify Apple identifies the expected Mac/model."
echo "4. Compare the result with this report."
...
echo "Build              : ${OS_BUILD:-Unknown}"
echo "Architecture       : ${ARCH:-Unknown}"

if [ "$ARCH" = "arm64" ]; then pass "Architecture" "Apple silicon (arm64)"; else warn "Architecture" "${ARCH:-Unknown}"; fi
OS_MAJOR="$(echo "$OS_VERSION" | cut -d. -f1)"
if [ -n "$OS_MAJOR" ] && [ "$OS_MAJOR" -ge 26 ]; then
  pass "macOS version" "${OS_NAME:-macOS} ${OS_VERSION} — Tahoe 26+"
  echo "ACTION: Check Parts & Service: System Settings -> General -> About -> Parts & Service"
  echo "Genuine = OK | Used = genuine Apple part previously used | Unknown/Unverified = CHECK"
  echo "Tahoe 26+ → Parts & Service can be checked; if the section is absent, macOS has not detected a qualifying repair."
else
  warn "macOS version" "${OS_NAME:-macOS} ${OS_VERSION:-Unknown} — older than Tahoe 26"
  echo "ACTION: If practical, update to macOS Tahoe 26+ and check Parts & Service."
fi
...
SMART="$(echo "$DISK_INFO" | awk -F': ' '/SMART Status/ {print $2; exit}' | xargs)"
...
if [ "$SMART" = "Verified" ]; then
  ...
  warn "SMART status" "$SMART"
else
  warn "SMART status" "Not reported"
fi
...
echo
echo
echo "=== 5. APPLE CHECK COVERAGE / SERIAL VERIFICATION ==="
CHECK_COVERAGE_URL="https://checkcoverage.apple.com/country-region?locale=en_US"
echo "Apple Check Coverage: $CHECK_COVERAGE_URL"
echo "Serial to enter: ${SERIAL:-Unknown}"
echo "Compare with: Model=${MODEL:-Unknown} | Identifier=${IDENTIFIER:-Unknown} | Serial=${SERIAL:-Unknown}"
echo "IMPORTANT: Verify Apple identifies the expected Mac/model and compare the serial with the physical Mac."
echo "This script does NOT scrape Apple's website; the check is manual."
if [ -n "$SERIAL" ]; then
  command -v pbcopy >/dev/null 2>&1 && printf "%s" "$SERIAL" | pbcopy
  command -v open >/dev/null 2>&1 && open "$CHECK_COVERAGE_URL"
  pass "Check Coverage" "Website opened; verify model/serial manually"
else warn "Check Coverage" "Serial unavailable"; fi

echo "=== 6. MDM / ORGANIZATION ENROLLMENT ==="
...
echo
echo "=== 10. RECENT CRASH / KERNEL PANIC REVIEW ==="
PANIC_FILES=0
...
echo
echo "=== 13. MANUAL TESTS STILL REQUIRED ==="
